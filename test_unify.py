...
class TestUnits(unittest.TestCase):
    def test_unify_quotes(self):
        self.assertEqual("'foo'", unify.unify_quotes('"foo"', preferred_quote="'"))
    ...
    def test_unify_quotes_should_avoid_some_cases(self):
        self.assertEqual('''"foo's"''', unify.unify_quotes('''"foo's"''', preferred_quote="'"))
...
class TestUnitsWithFstrings(unittest.TestCase):
    ...
    def test_unify_quotes(self):
        self.assertEqual("f'foo'", unify.unify_quotes('f"foo"', preferred_quote="'"))
    ...
    def test_unify_quotes_should_avoid_some_cases(self):
        self.assertEqual('''f"foo's"''', unify.unify_quotes('''f"foo's"''', preferred_quote="'"))
...
class TestUnitsWithRawStrings(unittest.TestCase):
    """Test for r-prefix raw string handling."""

    def test_unify_quotes(self):
        self.assertEqual("r'foo'", unify.unify_quotes('r"foo"', preferred_quote="'"))
    ...
    def test_unify_quotes_should_avoid_some_cases(self):
        self.assertEqual('''r"foo's"''', unify.unify_quotes('''r"foo's"''', preferred_quote="'"))

        self.assertEqual('''r"""\\t"""''', unify.unify_quotes('''r"""\\t"""''', preferred_quote="'"))
    ...
    def test_format_code_with_syntax_error(self):
        self.assertEqual(
            'foo(r"Tabs \t, new lines \n."\n', unify.format_code('foo(r"Tabs \t, new lines \n."\n', preferred_quote="'")
        )


class TestUnitsWithUnicodeStrings(unittest.TestCase):
    """Test for u-prefix unicode string handling."""

    def test_unify_quotes(self):
        self.assertEqual("u'foo'", unify.unify_quotes('u"foo"', preferred_quote="'"))
    ...
    def test_unify_quotes_should_avoid_some_cases(self):
        self.assertEqual('''u"foo's"''', unify.unify_quotes('''u"foo's"''', preferred_quote="'"))
...
class TestUnitsWithByteStrings(unittest.TestCase):
    ...
    def test_unify_quotes(self):
        self.assertEqual("b'foo'", unify.unify_quotes('b"foo"', preferred_quote="'"))
    ...
    def test_unify_quotes_should_avoid_some_cases(self):
        self.assertEqual('''b"foo's"''', unify.unify_quotes('''b"foo's"''', preferred_quote="'"))
...
""") as filename:
            ...
            self.assertEqual(
                unify._main(argv=['my_fake_program', filename], standard_out=output_file, standard_error=None),
                None,
            )
            self.assertEqual(
                """\
...
""") as filename:
            ...
            self.assertEqual(
                unify._main(
                    argv=['my_fake_program', '--check-only', filename], standard_out=output_file, standard_error=None
                ),
                1,
            )
            self.assertEqual(
...
""") as filename:
            ...
            self.assertEqual(
                unify._main(
                    argv=['my_fake_program', '--in-place', filename], standard_out=output_file, standard_error=None
                ),
                None,
            )
...
""") as filename:
            ...
            self.assertEqual(
                unify._main(
                    argv=['my_fake_program', '--in-place', '--check-only', filename],
...
""",
    ...
    def test_ignore_hidden_directories(self):
        with tempfile.TemporaryDirectory() as directory:
            with tempfile.TemporaryDirectory(prefix='.', dir=directory) as inner_directory:
                with temporary_file(
                    """\
if True:
    x = "abc"
...
def temporary_file(contents, directory='.', prefix=''):
    ...
    f = tempfile.NamedTemporaryFile(suffix='.py', prefix=prefix, delete=False, dir=directory)
    try:
